...
from pyviewer.utils import float_flip_lsb, normalize_image_data
...
class TextureWindowUnit:
    ...
    def dispose_texture(self) -> None:
        """Dispose OpenGL texture. This is called when the window is closed or the texture is no longer needed.
        """
...
# ------------------------------------------------------------------------------------------------------------
# MultiTextureDockingViewer


class MultiTexturesDockingViewer:
    def __init__(
        self,
        name: str,
    ...
    def keydown(self, key: imgui.Key):
    ...
    def setup_layout(self) -> hello_imgui.DockingParams:
        ...
        # For texture windows
        for key, texture_window in self.texture_windows.items():
    ...
    def load_fonts(self):
        ...
        if self.load_font_awesome:
            ...
            self.fonts.append(hello_imgui.load_font("fonts/Font_Awesome_6_Free-Solid-900.otf", size, font_loading_params_title_icons))
